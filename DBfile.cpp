...
int DBfile::getNumberOfLines(string fileName) {
	string line;
	int counter = 0;
...
void DBfile::addLine(vector<string> fields, vector<int> lengthFields) {
	...
	else if (getNumberOfLines() >= 1) {
		fout << "\n";
		fout << setw(5) << left << getNumberOfLines();
		for (int i = 0; i < fields.size(); i++) {
			fout << setw(lengthFields[i]) << left << fields[i];
		}
		//fout << "\n";
...
void DBfile::addColumn(string param, int widthCell) {
	...
	for (int i = 0; i < getNumberOfLines(); i++) {
		...
		if (i == 0) {
			...
			cout << "Value for last column of \"" + currentLine + "\" "; 
			//cin.get();
			//cin.clear();
			//cin.ignore(numeric_limits<streamsize>::max(), '\n');
			getline(cin, newValue);
			//cin >> newValue;
			if (widthCell < newValue.size()) {
...
void DBfile::removeColumn(vector<int> headerWordsLength) {
	...
	for (int i = 0; i < lines.size(); i++) {
		...
		if (i == lines.size() - 1) newFout << lines[i];
		else newFout << lines[i] << "\n";
	}
	newFout.close();
