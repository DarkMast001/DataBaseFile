...
void addLine() {
	...
	if (DBfile::getCurrentFileName() == "No current file!" || DBfile::getCurrentFileName() == "oops, you removed all files :)\n") {
		...
		for (int i = 0; i < countFields; i++) {
			...
			if (lengthField + 2 <= fieldName.size()) {
...
void removeLine() {
	...
	/*if (find(fileNameOfId.begin(), fileNameOfId.end(), currentFile) != fileNameOfId.end()) {
		auto index = find(fileNameOfId.begin(), fileNameOfId.end(), currentFile);
		int attachedFilePos = distance(attachedFiles.begin(), index);
	}*/
